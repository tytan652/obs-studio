...
static bool set_config(struct ffmpeg_output *stream)
{
	...
	if (!service)
		return false;
	config.url = obs_service_get_info(OBS_SERVICE_SERVER_URL, service);
	config.username = obs_service_get_info(OBS_SERVICE_USERNAME, service);
	config.password = obs_service_get_info(OBS_SERVICE_PASSWORD, service);
	config.stream_id = obs_service_get_info(OBS_SERVICE_STREAM_ID, service);
	config.encrypt_passphrase =
		obs_service_get_info(OBS_SERVICE_ENCRYPT_PASSPHRASE, service);
