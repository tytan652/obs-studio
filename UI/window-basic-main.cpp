	...
	: OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	ads::CDockManager::setConfigFlag(
		ads::CDockManager::FloatingContainerForceQWidgetTitleBar,
	...
	mixerDock =
		new OBSAdvDock(QTStr("Mixer"), MIXER_DOCK_NAME, mixerWidget);
	dockArea = dockManager->addDockWidget(ads::RightDockWidgetArea,
					      mixerDock, dockArea);
	...
	controlsDock = new OBSAdvDock(QTStr("Basic.Main.Controls"),
				      CONTROLS_DOCK_NAME, controlsWidget);
	dockArea = dockManager->addDockWidget(ads::RightDockWidgetArea,
...
#define MAKE_CALLBACK(pred, method, log_action)                            \
	...
	streamingHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(!basic.outputHandler->StreamingActive() &&
				      basic.controlsWidget->ui->streamButton
					      ->isEnabled(),
	...
	recordingHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(!basic.outputHandler->RecordingActive() &&
				      !basic.controlsWidget->ui->recordButton
					       ->isChecked(),
	...
	pauseHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(
			!!basic.controlsWidget->pauseButton &&
				!basic.controlsWidget->pauseButton->isChecked(),
...
void OBSBasic::CreateSourcePopupMenu(int idx, bool preview)
{
	...
	if (idx != -1) {
		...
		popup.addAction(QTStr("Rename"), this,
				SLOT(EditSceneItemName()));
		popup.addAction(QTStr("Remove"), this,
				SLOT(RemoveSourceActionriggered()));
		popup.addSeparator();
		...
		action = popup.addAction(
			...
			SLOT(SourcePropertiesActionTriggered()));
...
{
	sourcesWidget->ToggleToolbarVisibility(visible);
	scenesWidget->ToggleToolbarVisibility(visible);
...
{
	if (!controlsWidget->pauseButton || !outputHandler ||
...
void OBSBasic::on_customContextMenuRequested(const QPoint &pos)
{
	...
	if (className && strstr(className, "Dock") != nullptr &&
		...
		if (objName.compare("scenesDock") == 0) {
			ScenesContextMenuRequested(globalPos);
		} else if (objName.compare("sourcesDock") == 0) {
			SourcesContextMenuRequested(globalPos);
