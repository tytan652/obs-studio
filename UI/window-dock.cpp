...
void OBSAdvDock::SetupConnections()
{
	...
	connect(this, SIGNAL(closed()), SLOT(ShowCloseWarning()));

	// Avoid showing the warning message if closing the dock from the dock menu
	disconnect(toggleViewAction(), SIGNAL(triggered(bool)), this,
		   SLOT(toggleView(bool)));
	connect(toggleViewAction(), SIGNAL(triggered(bool)),
		SLOT(ToggleViewNoWarning(bool)));

	// Allow the dock having his position and size reset when needed
	connect(this, SIGNAL(viewToggled(bool)), SLOT(ResetPos(bool)));

	/* Since the dock manager toggle the dock once added
	 * connections are setup with the dock manager when it happen */
	connect(this, SIGNAL(viewToggled(bool)), SLOT(SetupWithDockManager()));
}
...
void OBSAdvDock::SetupWithDockManager()
{
	...
	disconnect(this, SIGNAL(viewToggled(bool)), this,
		   SLOT(SetupWithDockManager()));
}
