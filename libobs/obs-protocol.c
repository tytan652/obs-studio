...
bool obs_protocol_get_video_codec_agnostic(const obs_protocol_t *protocol)
{
	return obs_protocol_valid(protocol, "obs_protocol_get_codec_agnostic")
		       ? protocol->info.video_codec_agnostic
		       : false;
}

bool obs_protocol_get_audio_codec_agnostic(const obs_protocol_t *protocol)
{
	return obs_protocol_valid(protocol, "obs_protocol_get_codec_agnostic")
		       ? protocol->info.audio_codec_agnostic
		       : false;
}

void obs_protocol_get_supported_video_codecs(const obs_protocol_t *protocol,
					     ...
					     size_t *count)
{
	...
	if (!obs_ptr_valid(codecs, "obs_protocol_supported_video_codecs"))
		return;
	if (!obs_ptr_valid(count, "obs_protocol_supported_video_codecs"))
		return;

	*codecs = NULL;
	*count = 0;

	if (protocol->info.get_supported_video_codecs)
		protocol->info.get_supported_video_codecs(
			protocol->context.data, codecs, count);
}

void obs_protocol_get_supported_audio_codecs(const obs_protocol_t *protocol,
					     struct obs_protocol_codec **codecs,
					     size_t *count)
{
	if (!obs_protocol_valid(protocol,
	...
	if (!obs_ptr_valid(codecs, "obs_protocol_supported_audio_codecs"))
		return;
	if (!obs_ptr_valid(count, "obs_protocol_supported_audio_codecs"))
		return;

	*codecs = NULL;
	*count = 0;

	if (protocol->info.get_supported_audio_codecs)
		protocol->info.get_supported_audio_codecs(
			protocol->context.data, codecs, count);
}
