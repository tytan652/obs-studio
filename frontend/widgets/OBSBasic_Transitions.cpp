...
void OBSBasic::InitDefaultTransitions()
{
	...
	for (OBSSource &tr : defaultTransitions) {
		...
		trsNameUuid.insert({obs_source_get_name(tr), uuid});
...
{
	if (trsNameUuid.find(name) == trsNameUuid.end())
		return nullptr;

	return transitions[trsNameUuid[name]];
}
...
{
	if (transitions.find(currentTransitionUuid) == transitions.end())
		return nullptr;

	return transitions[currentTransitionUuid];
}
...
void OBSBasic::on_transitionRemove_clicked()
{
	...
	tr = transitions[currentTransitionUuid];
	...
	trsNameUuid.erase(obs_source_get_name(tr));
	transitions.erase(currentTransitionUuid);
	SetCurrentTransition(std::prev(transitions.end())->first);
...
void OBSBasic::RenameTransition(OBSSource transition)
{
	...
	if (trsNameUuid.find(oldName) == trsNameUuid.end() ||
	    transitions.find(trsNameUuid[oldName]) == transitions.end())
		return;

	trsNameUuid.erase(oldName);
	trsNameUuid.insert({name, obs_source_get_uuid(source)});
...
QMenu *OBSBasic::CreatePerSceneTransitionMenu()
{
	...
	auto setTransition = [this](QAction *action) {
		...
		if (uuid.empty() || transitions.find(uuid) == transitions.end()) {
	...
	auto addAction = [&](const std::string &uuid = "") {
		...
		if (!uuid.empty()) {
			if (transitions.find(uuid) == transitions.end())
				return;
			name = obs_source_get_name(transitions[uuid]);
		}
...
QMenu *OBSBasic::CreateTransitionMenu(QWidget *parent, QuickTransition *qt)
{
	...
	for (const auto &pair : transitions) {
		...
		action = menu->addAction(obs_source_get_name(pair.second));
		action->setProperty("transition_uuid", QString::fromStdString(pair.first));
...
void OBSBasic::AddQuickTransition()
{
	...
	if (!fadeToBlack && (transitions.find(transitionUuid) == transitions.end()))
		return;

	transition = fadeToBlack ? OBSSource(fadeTransition) : transitions[transitionUuid];
...
void OBSBasic::QuickTransitionChange()
{
	...
	if (qt) {
		...
		tr = fadeToBlack ? OBSSource(fadeTransition) : transitions[transitionUuid];
